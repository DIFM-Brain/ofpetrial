...
When the trial design is created, the downloaded files also include a shapefile labeled "ab-lines-farmer-harvester", which the farmer should use to center the harvester at harvest time. Figure \@ref(fig:machine-alignment-harvester) shows the harvester and its AB-line along with all other machinery inside a trial plot. If the harvester is correctly centered, then the `r as.character(english(machine_table[nrow(machine_table), ]$number_in_plot))` `r if(units == "metric"){machine_table[nrow(machine_table), ]$width}else{conv_unit(machine_table[nrow(machine_table), ]$width, "m", "ft")}`-_length-unit_ harvester swaths will lie neatly within each `r text_plot_width(all_trial_info, units)`   
