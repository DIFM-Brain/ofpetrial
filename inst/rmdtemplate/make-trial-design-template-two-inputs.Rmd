...
rep_table_joint <- cbind(all_trial_info$trial_design[[1]],
               all_trial_info$trial_design[[2]]) %>%
  mutate(treatment = paste0(rate, "_", rate.1)) %>%
  group_by(treatment) %>%
...
knitr::kable(rep_table_joint,
  caption = paste0("Replicate of each Combination of", rep_table$input_name[[1]], " and ", rep_table$input_name[[2]], " Rate"),
  booktabs = TRUE)
```
