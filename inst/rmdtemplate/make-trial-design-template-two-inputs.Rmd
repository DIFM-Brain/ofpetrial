...
Figure \@ref(fig:td-map) depicts the trial design made for the _trial-name_ trial.  Note that in the interior of the _field-size_-_land-unit_ field contains a grid of `r text_plot_num_length(all_trial_info, units)` Each plot is assigned one of `r text_rate_number(all_trial_info)` The variable rate `r trial_text_machinery_names_lower(machine_table)` `r text_plant_apply(all_trial_info)` `r trial_text_inputs(all_trial_info)` in each plot at a rate close to its targeted rate. `r text_base_rate(all_trial_info, units)`

```{r rates-table, echo=FALSE, include = TRUE}
rate_table1 <- data.table(paste0(all_trial_info$input_name[[1]], " Rates") =  all_trial_info$tgt_rate_original[[1]],
                               paste0("Rates in ", all_trial_info$input_type[[1]], "Equivalent") = all_trial_info$tgt_rate_equiv[[1]],
                        "Rates in Equivalent including Base Rate" = all_trial_info$total_equiv[[1]])

rate_table2 <- data.table(paste0(all_trial_info$input_name[[2]], " Rates") =  all_trial_info$tgt_rate_original[[2]],
                               paste0("Rates in ", all_trial_info$input_type[[2]], "Equivalent") = all_trial_info$tgt_rate_equiv[[2]],
                        "Rates in Equivalent including Base Rate" = all_trial_info$total_equiv[[2]])


knitr::kable(list(rep_table$rates_table[[1]], rep_table$rep_table[[2]]),
...
Figure \@ref(fig:trial-plot) displays the trial plots for the two inputs, with `r text_plot_width(all_trial_info, units)`.
